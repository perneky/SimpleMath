...
public:
  ...
  virtual size_t Validate( const EvaluateContext& context ) const override
  {
    ...
    if ( ldim != rdim && rdim != 1 )
      throw ValidationError( ErrorType::InvalidDimensions, "For multiplication operator, the dimension of operands should be equal, or one for the right operand ( %d, %d ).", int( ldim ), int( rdim ) );
